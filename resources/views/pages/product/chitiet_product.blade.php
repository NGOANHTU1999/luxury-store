...
@section('content')
    ...
    <section class="breadcrumb-section set-bg owl-carousel owl-loaded owl-drag">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <i class="breadcrumb-item"><a href="{{URL::to('/trang-chu')}}">Home</a></i>
                    <i class="breadcrumb-item"><a
                            href="{{URL::to('/thuong-hieu-san-pham/'.$brand_slug)}}">{{$product_brand}}</a>
                    </i>
                    <i class="breadcrumb-item"><a
                            href="{{URL::to('/danh-muc-san-pham/'.$cate_slug)}}">{{$product_cate}}</a>
                    </i>

                    <i class="breadcrumb-item active" aria-current="page">{{$meta_title}}</i>
                </div>
            </div>
        </div>
    </section>
                    ...
                    <div class="col-lg-6 col-md-6">
                        <div class="product__details__text">
                            ...
                            <div class="product__details__rating">
                                <ul class="list-inline rating" title="Average Rating">
                                    @for($count=1; $count<=5; $count++)
                                        ...
                                        @endphp
                                        <i title="star_rating" id="{{$value->product_id}}-{{$count}}"
                                           data-index="{{$count}}"
                                           data-product_id="{{$value->product_id}}" data-rating="{{$rating}}"
                                           class="rating"
                            ...
                            <form method="POST">
                                ...
                                <div
                                    class="product__details__price">{{number_format($value->product_price).' '.'VNĐ'}}</div>
                                <div class="buttons_added">
                                    <input class="minus is-form" type="button" value="-"/>
                                    <input name="qty" class="input-qty cart_product_qty_{{$value->product_id}}"
                                           max="1000" min="1" type="number" value="1"/>
                                    <input type="hidden" name="productid_hidden" value="{{$value->product_id}}"/>
                                    <input class="plus is-form" type="button" value="+"/>
                                </div>
                                <input type="button" class="primary-btn add-to-cart"
                                       data-id_product="{{$value->product_id}}" value="{{__('Add To Cart')}}"
                                       name="add-to-cart">
                            </form>

                            <ul>
                                <li><b>{{__('Condition')}}:</b> <span><?php
                                        if ($value->product_qty > 0) {
                                        ?>
                                    <span>{{__('Stocking')}}</span>
                                    <?php
                                        }else{
                                        ?>
                                    ...
                                    <?php
                                        ...
                                        ?></span></li>
                                <li><b>{{__('Product Code')}}:</b> <span>{{($value->product_id)}}</span></li>
                                <li><b>{{__('Category')}}:</b>
                                    <?php
                                    $language = Session::get('language');
                                    if ($language == 'en') {
                        ...
                        <ul class="nav nav-tabs" role="tablist">
                            ...
                            <li class="nav-item active">
                                <a class="nav-link " data-toggle="tab" href="#tabs-1" role="tab"
                                   aria-selected="true">{{__('Describe')}}</a>
                            </li>

                            <li class="nav-item ">
                                <a class="nav-link" data-toggle="tab" href="#tabs-2" role="tab"
                                   aria-selected="false">{{__('Comment')}} <span></span></a>
                            </li>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane " id="tabs-2" role="tabpanel">
                                <div class="product__details__tab__desc">
                                    ...
                                    <form action="#">
										<span>
											<input style="width:20%;margin-left: 10px;height: 35px;" type="text" class="comment_name"
                                                   placeholder="Your Name"/>

										</span>
                                        <p></p>
                                        <textarea name="comment" class="comment_content" placeholder="Comment Content"
                                                  style="width: 98%; margin-left: 10px; height: 180px;"></textarea>
                                        <div id="notify_comment"></div>

                                        <button style="background-color: #fa4251; width: 8%; margin-left: 10px; border-radius: 10px; height:40px;"
                                                type="button" class="send-comment">
                                            Send
                                        </button>
                                        <p></p>
                                    </form>
                        ...
                        <div class="product__item">
                            <form>
                                ...
                                <div class="product__item__pic set-bg"
                                     data-setbg="{{URL::to('public/upload/product/'.$lienquan->product_image)}}">
                                    <ul class="product__item__pic__hover">
                                        ...
                                        if ($language == 'en') {
                                        ?>
                                        <input type="hidden" value="{{$lienquan->product_name_en}}"
                                        ...
                                        <input type="hidden" value="{{$lienquan->product_qty}}"
                                               class="cart_product_quantity_{{$lienquan->product_id}}">
                                        <input type="hidden" value="1"
                                               class="cart_product_qty_{{$lienquan->product_id}}">
                                        <li><a href="#"><i class="fa fa-heart"></i></a></li>
                                        <li><a href="{{URL::to('chi-tiet-san-pham/'.$lienquan->product_slug)}}"><i
                                                    class="fa fa-info"></i></a>
                                        <li><a type="button" class="add-to-cart"
                                               data-id_product="{{$lienquan->product_id}}" name="add-to-cart"><i
                            ...
                            <div class="featured__item__text">
                                ...
                                ?>
                                <h6>
                                    <a href="{{URL::to('chi-tiet-san-pham/'.$lienquan->product_slug)}}">{{($lienquan->product_name_en)}}</a>
                                </h6>
                                <?php
                                }else{
                                ?>
