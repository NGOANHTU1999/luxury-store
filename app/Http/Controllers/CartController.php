...
class CartController extends Controller
{
    ...
    public function update_cart(Request $request)
    {
        ...
        if ($cart == true) {
            foreach ($data['cart_qty'] as $key => $qty) {
                ...
                foreach ($cart as $session => $val) {
                    ...
                    if ($val['session_id'] == $key && $qty < $cart[$session]['product_quantity']) {
                        ...
                        $message .= '<p style="color:green;">' . $i . '/ Update Quantity ' . $cart[$session]['product_name'] . ' Successfully</p>';
                    } elseif ($val['session_id'] == $key && $qty > $cart[$session]['product_quantity']) {
                        $message .= '<p style="color:red;">' . $i . '/ Quantity ' . $cart[$session]['product_name'] . ' Must Be Smaller ' . $cart[$session]['product_quantity'] . '</p>';
                    }
                }
            }
            Session::put('cart', $cart);
            ...
            return redirect()->back()->with('message', 'Failed Quantity Update');
        }
    }
    ...
    public function gio_hang(Request $request)
    {
        ...
        return view('pages.cart.cart_ajax')->with('category', $cate_product)->with('brand',
            $brand_product)->with('meta_desc', $meta_desc)->with('meta_keywords', $meta_keywords)->with('meta_title',
            $meta_title)->with('url_canonical', $url_canonical);
    }
    ...
    public function unset_coupon(Request $request)
    {
        ...
        if ($coupon == true) {
            ...
            return redirect()->back()->with('message', 'Delete Promo Code Successfully');
        }
    }
    ...
    public function check_coupon(Request $request)
    {
        ...
        if ($coupon && $coupon->coupon_qty > 0) {
            if (date_format($date_end, "m") >= $month_now && date_format($date_end,
                    "Y") >= $year_now && $coupon->coupon_date_end >= $today) {
                ...
                if ($count_coupon > 0) {
                    ...
                    Session::save();
                    return redirect()->back()->with('message', 'Add Successful Promo Code');
                }
            } else {
                return redirect()->back()->with('error', 'Promo Code Invalid Or Expired');
            }
        } else {
            return redirect()->back()->with('error', 'Promo Code Incorrect Or Expired');
        }
    ...
    public function save_coupon(AddCouponRequests $request)
    {
        ...
        $coupon->save();
        Session::put('message', 'Add Successful Promo Code');
    ...
    public function update_coupon(AddCouponRequests $request, $coupon_id)
    {
        ...
        Session::put('message', 'Update Promotion Code Successful');
        return Redirect::to('all-coupon');
    }
    ...
    public function delete_coupon($coupon_id)
    {
        ...
        Session::put('message', 'Delete Promo Code Successfully');
        return Redirect::to('all-coupon');
    }
            ...
            $output .= '
                <tbody>
                          <tr>
                            ...
                            <td contenteditable data-feeship_id="' . $fee->fee_id . '" class="fee_ship_edit">' . number_format($fee->fee_ship,
                    0, ',', '.') . '</td>
                          </tr>
                    </tbody
                    ';
