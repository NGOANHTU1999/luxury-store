...
class Post extends Controller
{
    ...
    public function save_post(PostRequests $request)
    {
        ...
        if ($get_image) {
            ...
            Session::put('message', 'Please add a picture');
            return redirect()->back();
        }
    }
    ...
    {
        $this->AuthenLogin();
        $all_post = Posts::orderBy('post_id', 'DESC')->paginate(20);
...
//////////////////////////////////////////////////////////////////frontend//////////////////////////////////////////////////////////
    public function tin_tuc(Request $request)
    {
        ...
        $meta_desc = "Watch-Related Bloggers";
        $meta_keywords = "Blogs, Watches, Deals, Discounts";
        $meta_title = "Blog || Luxury - Store";
        $url_canonical = $request->url();
        $post = Posts::where('post_status', 1)->orderBy('post_id', 'DESC')->paginate(6);
        return view('pages.blog.tin_tuc')->with(compact('category', 'brand', 'meta_desc', 'meta_keywords', 'meta_title',
            'url_canonical', 'category_blog', 'post'));
    }
    ...
    public function danh_muc_bai_viet(Request $request, $post_slug)
    {
        ...
        return view('pages.blog.blog')->with(compact('category', 'brand', 'meta_desc', 'meta_keywords', 'meta_title',
            'url_canonical', 'category_blog', 'post', 'post_new'));
    }
    ...
    public function bai_viet(Request $request, $post_slug)
    {
        ...
        $post_view->save();
        //san pham lien quan
        $post_related = Posts::with('cate_post')->where('post_status', 1)->where('cate_blog_id',
            $cate_blog_id)->whereNotIn('post_slug', [$post_slug])->take(3)->get();
        $post_new = Posts::where('post_status', 1)->orderBy('post_id', 'DESC')->limit(3)->get();
        return view('pages.blog.bai_viet')->with(compact('category', 'brand', 'meta_desc', 'meta_keywords',
            'meta_title', 'url_canonical', 'category_blog', 'post', 'post_related', 'post_new'));
    }
}
