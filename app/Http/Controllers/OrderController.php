...
class OrderController extends Controller
{
    ...
    public function update_qty(Request $request)
    {
        $this->AuthenLogin();
        $data = $request->all();
        $order_details = OrderDetails::where('product_id', $data['order_product_id'])->where('order_code',
            $data['order_code'])->first();
    ...
    public function print_order_convert($checkout_code)
    {
        ...
        $output .= '<style>body{
                ...
                <table class="Table">
                  <h1><center>Luxury - Store</center></h1>
        ...
        $output .= '
                <tr>
                    <td colspan="12" style="text-align:right;">
                      <p>Total Decrease: ' . $coupon_echo . '</p>
                      <p>Shipping fee: ' . number_format($product->product_feeship, 0, ',', '.') . ' ' . 'VNĐ' . '</p>
                      <p>Payment : ' . number_format($total_coupon + $product->product_feeship, 0, ',',
                '.') . ' ' . 'VNĐ' . '</p>
                    </td>
                </tr>';
        $output .= '
                  ...
                  <table>
                    <thead>
                      <tr>
                        <th width="200px">Invoice printer</th>

                        <th width="800px">Recipient</th>
                      </tr>
                    </thead>
                    <tbody>';
