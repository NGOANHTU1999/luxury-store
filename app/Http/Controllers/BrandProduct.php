...
class BrandProduct extends Controller
{
    ...
    {
        $this->AuthenLogin();
        $all_brand_product = DB::table('tbl_brand_product')->paginate(20);
    ...
    public function edit_brand_product($brand_product_id)
    {
        ...
        $manager_brand_product = view('admin.Brand.edit_brand_product')->with('edit_brand_product',
            $edit_brand_product);
    ...
    public function show_brand_home(Request $request, $brand_slug)
    {
        ...
        $brand_by_id = DB::table('tbl_product')->join('tbl_brand_product', 'tbl_product.brand_id', '=',
            'tbl_brand_product.brand_id')->where('tbl_brand_product.brand_slug', $brand_slug)->get();

        //xô thương hiệu
        $brand_name = DB::table('tbl_brand_product')->where('tbl_brand_product.brand_slug',
            $brand_slug)->limit(1)->get();
        ...
        return view('pages.brand.show_brand')->with('category', $cate_product)->with('brand',
            $brand_product)->with('brand_by_id', $brand_by_id)->with('brand_name', $brand_name)->with('meta_desc',
            $meta_desc)->with('meta_keywords', $meta_keywords)->with('meta_title', $meta_title)->with('url_canonical',
            $url_canonical);
    }
}
