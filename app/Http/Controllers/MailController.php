...
class MailController extends Controller
{
    ...
    public function send_coupon($coupon_qty, $coupon_name, $coupon_condition, $coupon_number, $coupon_code)
    {
        ...
        $title_mail = "Super attractive promo code, limited number of codes" . ' ' . $now;
        $data = [];
        ...
        Mail::send('admin.Email.send_coupon', ['coupon' => $coupon],
            function ($message) use ($title_mail, $data, $coupon) {
                $message->to($data['email'])->subject($title_mail);//send this mail with subject
                $message->from($data['email'], $title_mail);//send  from this mail
            });
        return redirect()->back()->with('message', 'Send promo code to successful customer');
